...
/**
 * Get recent items from dashboard API
 * @param {number} limit - Number of items to fetch
 * @param {string} sortBy - Sort field (added_date, ratio, size, title)
 * @param {string} sortOrder - Sort order (asc, desc)
 * @returns {Promise<Array>} Array of recent items
 */
export const getRecentItems = async (limit = 20, sortBy = 'added_date', sortOrder = 'desc') => {
  try {
    const response = await servarrHubClient?.get(`/dashboard/recent-items?limit=${limit}&sort_by=${sortBy}&sort_order=${sortOrder}`);
...
/**
 * Get all library items (recent additions)
 * @returns {Promise<Array>} Array of library items
 */
export const getLibraryItems = async () => {
  try {
    const response = await servarrHubClient?.get('/library/items');
    return response?.data || [];
  } catch (error) {
 ...
 * @returns {Promise<Object|null>} Created library item or null
 */
export const addLibraryItem = async (item) => {
  try {
    const response = await servarrHubClient?.post('/library/items', item);
    return response?.data || null;
  } catch (error) {
    console.error('Error adding library item:', error?.message);
    return null;
  }
};
...
export const deleteLibraryItem = async (id) => {
  try {
    await servarrHubClient?.delete(`/library/items/${id}`);
    return true;
  } catch (error) {
...
export const getMediaDetail = async (id) => {
  ...
  const mockMovies = {
    '1': {
      ...
      year: 1994,
      mediaType: 'movie',
      image: "https://img.rocket.new/generatedImages/rocket_gen_img_1e7c1ee7d-1766845815525.png",
      imageAlt: 'The Shawshank Redemption movie poster showing Andy Dufresne',
      backdropImage: "https://img.rocket.new/generatedImages/rocket_gen_img_1d395bfc4-1764681696426.png",
    ...
    '2': {
      ...
      year: 2010,
      mediaType: 'movie',
      image: "https://img.rocket.new/generatedImages/rocket_gen_img_1dbfd00a0-1764665583149.png",
      imageAlt: 'Inception movie poster featuring Leonardo DiCaprio',
      backdropImage: "https://images.unsplash.com/photo-1705948353989-ecb64769f1ef",
  ...
  const mockTVShows = {
    '3': {
      ...
      year: 2008,
      mediaType: 'tv',
      image: "https://img.rocket.new/generatedImages/rocket_gen_img_132f74d43-1767463020900.png",
    ...
    '4': {
      ...
      year: 2016,
      mediaType: 'tv',
      image: "https://img.rocket.new/generatedImages/rocket_gen_img_18fc9d2c2-1770494626079.png",
      imageAlt: 'Stranger Things TV show poster with main cast',
      backdropImage: "https://img.rocket.new/generatedImages/rocket_gen_img_1492b7b67-1767527751487.png",
