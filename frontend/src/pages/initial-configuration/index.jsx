...
import { getServiceConfigurations, saveServiceConfiguration, testServiceConnection } from '../../services/configService';
    ...
    if (!isValid) {
      const errorStatus = {
        ...
        details: usesApiKey 
          ...
          : usesCredentials
          ? 'Please provide URL, username, and password' :'Please provide a valid URL'
      };
      setTestStatuses(prev => ({
        ...prev,
          ...
          <div className="mb-6 md:mb-8">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-lg flex items-center justify-center bg-primary/10">
                <Icon name="Settings" size={24} color="var(--color-primary)" className="md:w-7 md:h-7" />
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-foreground">Initial Configuration</h1>
          ...
          <div className="bg-card border border-border rounded-lg p-4 md:p-6 shadow-elevation-2">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                <Icon name="Save" size={24} color="var(--color-primary)" className="flex-shrink-0 mt-1" />
                <div>
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground">
                    {allServicesConfigured 
                      ? 'All services are configured and ready. Click save to proceed to the dashboard.' :'Test all service connections before saving your configuration.'}
                  </p>
                </div>
              </div>
              <Button
          ...
          <div className="mt-6 md:mt-8 p-4 md:p-6 bg-muted/30 border border-border rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm md:text-base font-semibold text-foreground mb-2">Need Help?</h4>
