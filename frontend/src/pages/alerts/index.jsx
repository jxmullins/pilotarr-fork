...
const Alerts = () => {
  ...
  const [alerts, setAlerts] = useState([
    {
      ...
      details: 'Connection timeout after 30 seconds. Please verify the API URL and network connectivity.',
      timestamp: new Date(Date.now() - 300000),
      dismissed: false,
      suggestions: ['Check API URL configuration', 'Verify network connectivity', 'Ensure Radarr service is running']
    },
    {
      id: 2,
      type: 'api',
      ...
      details: 'Current API usage is at 85% of the rate limit. Consider reducing request frequency.',
      timestamp: new Date(Date.now() - 600000),
      dismissed: false,
      suggestions: ['Increase refresh interval', 'Review automated tasks', 'Contact Sonarr support for rate limit increase']
    },
    {
      id: 3,
      type: 'request',
      ...
      dismissed: false,
      suggestions: ['Verify TMDB ID is correct', 'Check Jellyseerr logs', 'Retry the request manually']
    },
    {
      id: 4,
      type: 'system',
      ...
      dismissed: false,
      suggestions: ['Check active transcoding sessions', 'Review server resources', 'Consider hardware upgrade']
    },
    {
      id: 5,
      type: 'connection',
      ...
      dismissed: false,
      suggestions: ['Restart Sonarr service', 'Check database server status', 'Review database logs']
    },
    {
      id: 6,
      type: 'api',
      ...
      details: 'Your Radarr API key has been in use for over 90 days. Consider rotating for security.',
      timestamp: new Date(Date.now() - 2400000),
      dismissed: false,
      suggestions: ['Generate new API key in Radarr settings', 'Update API key in ServarrHub', 'Test connection after update']
    }
  ]);
  ...
  const tabs = [
    { id: 'all', label: 'All Alerts', count: alerts?.filter(a => !a?.dismissed)?.length },
    { id: 'connection', label: 'Connection Failures', count: alerts?.filter(a => a?.type === 'connection' && !a?.dismissed)?.length },
    { id: 'api', label: 'API Errors', count: alerts?.filter(a => a?.type === 'api' && !a?.dismissed)?.length },
    { id: 'request', label: 'Failed Requests', count: alerts?.filter(a => a?.type === 'request' && !a?.dismissed)?.length },
    { id: 'system', label: 'System Warnings', count: alerts?.filter(a => a?.type === 'system' && !a?.dismissed)?.length }
  ];
  ...
  const handleDismissSelected = () => {
    ...
    setAlerts(alerts?.map(a => selectedAlerts?.includes(a?.id) ? { ...a, dismissed: true } : a));
    setDismissedAlerts([...dismissedAlerts, ...dismissedItems?.map(item => ({ ...item, dismissedAt: now }))]);
    setSelectedAlerts([]);
  };
  ...
  const handleMarkAllRead = () => {
    ...
    setDismissedAlerts([...dismissedAlerts, ...activeAlerts?.map(item => ({ ...item, dismissedAt: now }))]);
    setSelectedAlerts([]);
  };
  ...
  const handleRestoreAlert = (alert) => {
    setAlerts([...alerts?.filter(a => a?.id !== alert?.id), { ...alert, dismissed: false }]);
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600">Monitor and manage system notifications across all Servarr services</p>
            </div>
            ...
            {tabs?.map(tab => (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-4 font-medium whitespace-nowrap transition-colors border-b-2 ${
                  activeTab === tab?.id
                    ? 'border-orange-500 text-orange-600 bg-orange-50' :'border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                }`}
              >
                {tab?.label}
                {tab?.count > 0 && (
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                    activeTab === tab?.id
                      ? 'bg-orange-500 text-white' :'bg-gray-200 text-gray-700'
                  }`}>
                    {tab?.count}
                  </span>
                )}
